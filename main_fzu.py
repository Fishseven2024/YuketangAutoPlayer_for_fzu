#在原作者的代码基础上改为fzu长江雨课堂可以运行的版本
#此代码适用于2025秋_fzu研究生一年级
...
# 修改下面这行，加上 changjiang 或者直接判断 v2
if 'www.yuketang.cn' in homePageURL or 'changjiang.yuketang.cn' in homePageURL:
    IS_COMMONUI = True
...
def finish1video():
    if IS_COMMONUI:
        try:
            # 尝试点击“学习日志”标签，如果找不到也没关系，说明可能已经在列表页了
            scoreList = driver.find_element(By.ID, 'tab-student_school_report')
            scoreList.click()
            print("已点击切换到学习日志/成绩单标签")
        except:
            print("未找到'学习日志'标签，假设当前已在列表页，继续执行...")
    ...
    # =========== 修改开始 ===========
    try:
        # 1. 先强制睡3秒，防止网页还没白屏加载就检测
        sleep(3)
        # 2. 将等待时间从 10 改为 60，并检测 video 标签是否存在
        WebDriverWait(driver, 60).until(
            lambda x: driver.execute_script('return document.querySelector("video") != null'))
        # 3. 赋值给 js 变量 video
        driver.execute_script('video = document.querySelector("video");')
    except Exception as e:
        print("错误: 60秒内未加载出视频元素，可能是网络卡顿或需要验证码。")
        # 关闭当前这个加载失败的窗口
        driver.close()
        # 切回目录页窗口
        driver.switch_to.window(driver.window_handles[0])
        # 返回 False 表示没播放成功，让主循环决定是否重试
        return False
    # =========== 修改结束 ===========
